...
public final class KokoroPhonemizer {
    ...
    /// Convert text to a phoneme (IPA) string using the language-appropriate phonemizer.
    /// Words are separated by single spaces; punctuation is mapped to phoneme symbols.
    func phonemeString(for text: String, language: String = "en") -> String {
        switch language {
        case "zh", "cmn", "chinese", "mandarin":
            return chinesePhonemizer.phonemize(text)
        case "ja", "japanese":
            return japanesePhonemizer.phonemize(text)
        case "it", "italian":
            return phonemizeWithDict(text, dict: PronunciationDicts.it, fallback: italianPhonemizer)
        case "fr", "french":
            return phonemizeWithDict(text, dict: PronunciationDicts.fr, fallback: frenchPhonemizer)
        case "es", "spanish":
            return phonemizeWithDict(text, dict: PronunciationDicts.es, fallback: spanishPhonemizer)
        case "pt", "portuguese":
            return phonemizeWithDict(text, dict: PronunciationDicts.pt, fallback: portuguesePhonemizer)
        case "hi", "hindi":
            return phonemizeWithDict(text, dict: PronunciationDicts.hi, fallback: hindiPhonemizer)
        default:
    ...
    public func tokenize(_ text: String, maxLength: Int = 510, language: String = "en") -> [Int] {
        let phonemes = phonemeString(for: text, language: language)
    ...
    /// The token-index span (into the id array, half-open) covering one phoneme "word".
    public struct PhonemeWordSpan: Sendable {
        /// Inclusive start index into the id array returned by `tokenizeWithWordSpans`.
        public let tokenStart: Int
        /// Exclusive end index.
        public let tokenEnd: Int
    }

    /// Like `tokenize`, but also returns the token-index span of each whitespace-delimited
    /// phoneme word, in source-word order. Produces the **identical** id sequence as
    /// `tokenize` so synthesis audio is unchanged — it only records word boundaries so
    /// per-token durations (`pred_dur`) can be grouped into word-level timestamps.
    ///
    /// A "word" is a maximal run of non-space phoneme tokens; trailing punctuation phonemes
    /// stay attached to the preceding word (they carry that word's tail duration).
    public func tokenizeWithWordSpans(
        _ text: String, maxLength: Int = 510, language: String = "en"
    ) -> (ids: [Int], wordSpans: [PhonemeWordSpan]) {
        let phonemes = phonemeString(for: text, language: language)

        var ids = [bosId]
        var spans: [PhonemeWordSpan] = []
        var currentStart: Int? = nil

        func closeWord() {
            if let start = currentStart, ids.count > start {
                spans.append(PhonemeWordSpan(tokenStart: start, tokenEnd: ids.count))
            }
            currentStart = nil
        }
        ...
        for char in phonemes {
            if char == " " {
                closeWord()
                // Preserve a space token if the model's vocab expects one (keeps ids
                // identical to `tokenize`); it belongs to no word.
                if let id = vocab[" "] { ids.append(id) }
                continue
            }
            let s = String(char)
            guard let id = vocab[s] else { continue }  // unknown chars dropped (as in tokenize)
            if currentStart == nil { currentStart = ids.count }
            ids.append(id)
        }
        closeWord()
        ...
        if ids.count > maxLength {
            ids = Array(ids.prefix(maxLength - 1)) + [eosId]
            // Drop spans that fall outside the truncated range.
            spans = spans.compactMap { span in
                guard span.tokenStart < maxLength - 1 else { return nil }
                let end = min(span.tokenEnd, maxLength - 1)
                return end > span.tokenStart ? PhonemeWordSpan(tokenStart: span.tokenStart, tokenEnd: end) : nil
            }
        }
