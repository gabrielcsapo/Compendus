...
public final class LazyImageAttachment: NSTextAttachment {
    ...
    public func loadImageIfNeeded() {
        ...
        if let cached = ReaderImageCache.shared.image(forPath: imageURL.path) {
            image = cached
        } else if let loaded = UIImage(contentsOfFile: imageURL.path) {
            ReaderImageCache.shared.setImage(loaded, forPath: imageURL.path)
            image = loaded
        }
    }
    ...
    public func loadImageAsync() async -> Bool {
        ...
        let loaded: UIImage? = await Task.detached(priority: .userInitiated) {
            if let cached = ReaderImageCache.shared.image(forPath: url.path) {
                return cached
            } else if let img = UIImage(contentsOfFile: url.path) {
                ReaderImageCache.shared.setImage(img, forPath: url.path)
                return img
            }
            return nil
        }.value
