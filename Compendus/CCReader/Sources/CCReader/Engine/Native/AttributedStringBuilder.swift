...
public class AttributedStringBuilder {
    ...
    public func build(from nodes: [ContentNode]) -> (NSAttributedString, OffsetMap, PlainTextToAttrStringMap) {
        ...
        if result.length > 0 && result.string.hasSuffix("\n") {
            ...
            // Same clamp for the plain-text→attr map, or read-along highlighting of the
            // final sentence maps 1 char past the string (out-of-bounds NSRange).
            if var last = plainTextMap.entries.last {
                let attr = last.attrStringRange
                let clampedLength = min(attr.length, result.length - attr.location)
                if clampedLength != attr.length {
                    last = PlainTextToAttrStringMap.Entry(
                        plainTextRange: last.plainTextRange,
                        attrStringRange: NSRange(location: attr.location, length: clampedLength)
                    )
                    plainTextMap.entries[plainTextMap.entries.count - 1] = last
                }
            }
        }
    ...
    private func appendImage(url: URL, alt: String?, hintWidth: CGFloat?,
        ...
        guard let intrinsicSize = ReaderImageCache.shared.imageDimensions(forPath: url.path) else {
        ...
        result.append(NSAttributedString(string: "\n"))
        // NOTE: `alt` is intentionally NOT rendered as a visible caption when the
        // image loads — it's fallback/accessibility text (shown as `[alt]` only
        // when the file is missing, above). A real visible caption arrives as
        // separate CCD `caption` blocks, mapped alongside the image.
    }
    ...
    private func generateVideoThumbnail(url: URL, poster: URL?,
        ...
        if let posterURL = poster, posterURL.isFileURL {
            if let cached = ReaderImageCache.shared.image(forPath: posterURL.path) {
                baseImage = cached
            } else if let loaded = UIImage(contentsOfFile: posterURL.path) {
                ReaderImageCache.shared.setImage(loaded, forPath: posterURL.path)
                baseImage = loaded
            }
        }
