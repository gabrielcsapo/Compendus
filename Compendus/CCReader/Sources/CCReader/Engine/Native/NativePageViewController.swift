...
public class NativePageViewController: UIViewController, UITextViewDelegate {
    ...
    public private(set) var isTwoPageMode: Bool = false
    ...
    public private(set) var highlightRanges: [(id: String, range: NSRange, color: UIColor)] = []

    /// Read-only access to the current highlights for composing temporary overlays.
    public var currentHighlightRanges: [(id: String, range: NSRange, color: UIColor)] {
        highlightRanges
    }

    /// Transient read-along (narration) highlight, rendered on the live page in
    /// addition to user highlights so listening keeps the real typeset page
    /// instead of replacing it with a plain karaoke block. `sentence` is the
    /// full spoken sentence (soft tint); `word` is the currently spoken word
    /// (stronger tint). Both are ranges in the full chapter attributed string.
    public private(set) var narrationSentenceRange: NSRange?
    public private(set) var narrationWordRange: NSRange?
    public var narrationTintColor: UIColor = .tintColor
    ...
    public var onPageChanged: ((_ page: Int, _ totalPages: Int) -> Void)?
    ...
    public var onFootnoteTapped: ((URL) -> Void)?
    ...
    public func showPageIndicator(text: String) {
    ...
    public func updateTextContainerInsets(_ insets: UIEdgeInsets) {
    ...
    public func configureLayout(twoPage: Bool) {
    ...
    /// Replace the full attributed string (e.g. after a theme-only color update)
    /// and re-render the current page without altering pagination or page index.
    public func updateAttributedString(_ attributedString: NSAttributedString) {
        self.fullAttributedString = attributedString
        showCurrentPage()
    }

    /// Navigate to a progression value (0.0–1.0) within the current chapter.
    public func showProgression(_ progression: Double) {
    ...
    private func renderPage(at pageIndex: Int, into targetTextView: UITextView, fullString: NSAttributedString) {
        ...
        fullString.enumerateAttribute(.attachment, in: safeRange, options: []) { value, _, _ in
            ...
            if let cached = ReaderImageCache.shared.image(forPath: lazy.imageURL.path) {
                lazy.image = cached
            } else {
                hasUncached = true
            }
    ...
    public func loadImagesAsync(for pageIndex: Int) {
    ...
    public func applyHighlights(_ highlights: [(id: String, range: NSRange, color: UIColor)]) {
        self.highlightRanges = highlights
        refreshHighlightsInPlace()
    }

    /// Set (or clear) the transient read-along narration highlight and refresh
    /// the visible page in place. No-op repaint is avoided when nothing changed
    /// so this is cheap to call on every word/sentence tick.
    public func applyNarrationHighlight(sentence: NSRange?, word: NSRange?) {
        let sameSentence = narrationSentenceRange == sentence
        let sameWord = narrationWordRange == word
        guard !(sameSentence && sameWord) else { return }
        narrationSentenceRange = sentence
        narrationWordRange = word
        refreshHighlightsInPlace()
    }
    ...
    private func applyHighlightsToPage(_ pageString: NSAttributedString,
        ...
        guard !highlightRanges.isEmpty
                || narrationSentenceRange != nil
                || narrationWordRange != nil else { return pageString }
        ...
        // Apply the read-along narration highlight on top: a soft tint behind the
        // whole spoken sentence, plus a stronger tint behind the active word so
        // listeners can follow along on the real page (karaoke in place).
        applyNarration(narrationSentenceRange, alpha: 0.18, to: mutable, pageRange: pageRange)
        applyNarration(narrationWordRange, alpha: 0.42, to: mutable, pageRange: pageRange)

        return mutable
    }

    /// Apply a single narration tint range (clipped to the page) to `mutable`.
    private func applyNarration(_ range: NSRange?, alpha: CGFloat,
                                to mutable: NSMutableAttributedString, pageRange: NSRange) {
        guard let range else { return }
        let overlap = NSIntersectionRange(range, pageRange)
        guard overlap.length > 0 else { return }
        let localRange = NSRange(location: overlap.location - pageRange.location, length: overlap.length)
        guard localRange.location >= 0,
              localRange.location + localRange.length <= mutable.length else { return }
        mutable.addAttribute(.backgroundColor, value: narrationTintColor.withAlphaComponent(alpha),
                             range: localRange)
    }

    /// Check if a given range (in full chapter attributed string coords) is visible on the current page(s).
    public func isRangeVisibleOnCurrentPage(_ range: NSRange) -> Bool {
    ...
    public private(set) var currentTheme: ReaderTheme?

    public func applyTheme(backgroundColor: UIColor, theme: ReaderTheme? = nil) {
    ...
    private func overlayFloatingImages(for pageIndex: Int, in targetTextView: UITextView) {
        ...
        for floatEl in floatingElements {
            ...
            if let cached = ReaderImageCache.shared.image(forPath: floatEl.imageURL.path) {
                floatImage = cached
            } else {
                ...
                Task.detached(priority: .userInitiated) {
                    if let loaded = UIImage(contentsOfFile: url.path) {
                        ReaderImageCache.shared.setImage(loaded, forPath: url.path)
                    }
                }
                floatImage = nil
            }
