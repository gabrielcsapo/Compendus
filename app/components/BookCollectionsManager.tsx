...
interface BookCollectionsManagerProps {
  ...
  /** "card" (default) renders a titled block; "inline" renders chips + add button in one row. */
  variant?: "card" | "inline";
}
...
export function BookCollectionsManager({
  ...
  const addControl = (
    <div className="relative">
      <button
        ...
        className="inline-flex items-center gap-1 px-3 py-1 text-sm rounded-full border border-dashed border-border text-foreground-muted hover:text-foreground hover:border-border-hover transition-colors"
      >
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 w-64 bg-surface border border-border rounded-lg shadow-xl z-50 overflow-hidden">
            {isLoading ? (
              ...
              <div className="p-4 text-center text-foreground-muted text-sm">
                {allCollections.length === 0 ? "No collections yet" : "Book is in all collections"}
              </div>
            ) : (
  ...
  const chipEls = collections.map((collection) => (
    <div
      key={collection.id}
  ...
  return (
    <div>
      ...
      {collections.length > 0 ? (
        <div className="flex flex-wrap gap-2">{chipEls}</div>
      ) : (
