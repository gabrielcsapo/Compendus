 ...
 */
export function calculateTotalPages(content: NormalizedContent, viewport: ViewportConfig): number {
  switch (content.type) {
    case "pdf":
    ...
    case "text":
      return calculateTextPages(content, viewport);
    default:
      return 1;
  }
}
 ...
 */
function calculateTextPages(content: TextContent, viewport: ViewportConfig): number {
  ...
  const charsPerPage = estimateCharsPerPage(viewport);
 ...
 */
function estimateCharsPerPage(viewport: ViewportConfig): number {
...
export function getPage(
  ...
  const totalPages = calculateTotalPages(content, viewport);
  const clampedPage = Math.max(1, Math.min(pageNum, totalPages));

  switch (content.type) {
    case "pdf":
      return getPdfPage(content, clampedPage, totalPages, bookId);
    case "comic":
      return getComicPage(content, clampedPage, totalPages, bookId);
    case "audio":
      return getAudioPage(content, clampedPage, totalPages, bookId);
    case "text":
      return getTextPage(content, clampedPage, totalPages, viewport, bookId);
    default:
...
function getPdfPage(
  ...
  return {
    ...
    position,
    endPosition,
    chapterTitle: findChapterTitle(content.toc, position),
  };
}
...
function getTextPage(
  ...
  if (content.isFixedLayout) {
    ...
    return {
      ...
      chapterTitle: chapter?.title,
      isFixedLayout: true,
      cssUrls: collectCssUrls([chapter], bookId),
    };
  }
  ...
  const { html, chapterTitle, overlappingChapters } = extractTextRange(content, startChar, endChar);
  ...
  return {
    ...
    endPosition,
    chapterTitle,
    cssUrls: collectCssUrls(overlappingChapters, bookId),
  };
}
...
function extractTextRange(
  ...
  for (const chapter of content.chapters) {
    ...
    if (chapterStartOffset === 0 && chapterEndOffset >= chapter.text.length) {
      ...
      htmlParts.push(`<p>${escapeHtml(textSubset)}</p>`);
    }
  }
...
export function calculateTocPageNumbers(
  ...
  return toc.map((entry) => ({
    ...entry,
    pageNum: getPageForPosition(content, entry.position, viewport),
    children: entry.children
      ? calculateTocPageNumbers(content, entry.children, viewport)
      : undefined,
  }));
}
  ...
  for (const chapter of content.chapters) {
    ...
    while (results.length < maxResults) {
      ...
      results.push({
        ...
        position,
        pageNum: getPageForPosition(content, position, viewport),
        chapterTitle: chapter.title,
      });
 ...
 */
function findChapterTitle(toc: TocEntry[], position: number): string | undefined {
  ...
  for (const entry of toc) {
    ...
    if (entry.children) {
      const childTitle = findChapterTitle(entry.children, position);
