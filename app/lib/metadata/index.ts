...
export async function searchAllSources(
  ...
  if (isbn) {
    ...
    const results = await Promise.allSettled([lookupGoogleBooksByISBN(isbn), lookupByISBN(isbn)]);

    const googleResult = results[0].status === "fulfilled" ? results[0].value : null;
    const olResult = results[1].status === "fulfilled" ? results[1].value : null;
    ...
    if (combined.length === 0) {
      ...
      return searchAllSources(`isbn:${isbn}`);
    }
